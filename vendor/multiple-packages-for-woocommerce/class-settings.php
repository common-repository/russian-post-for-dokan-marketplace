...
if ( ! class_exists( 'BE_Multiple_Packages_Settings' ) ) :
	...
	class BE_Multiple_Packages_Settings extends WC_Settings_Page {
		...
		public function __construct() {
			...
			add_action( 'woocommerce_admin_field_shipping_restrictions', [ $this, 'output_additional_settings' ] );
			add_filter( 'woocommerce_admin_settings_sanitize_option_shipping_restrictions', [
				$this,
				'sanitize_shipping_restrictions_field'
			], 15, 3 );
		...
		function get_settings( $current_section = '' ) {
			...
			return apply_filters( 'woocommerce_multi_packages_settings', [
					...
					[
						...
						'class'    => 'chosen_select',
						'desc_tip' => true,
						'options'  => apply_filters( 'be_packages_settings_type', [
							'shipping-method' => __( '3PL Shipping', 'bolder-multi-package-woo' ),
					...
					[ 'type' => 'sectionend', 'id' => 'multi_packages' ],
		...
		function output_additional_settings( $current_section = '' ) {
			...
			$shipping_classes = [ '' => 'No Shipping Class' ];
			$get_classes      = WC()->shipping->get_shipping_classes();
			foreach ( $get_classes as $key => $class ) {
				$shipping_classes[ $class->term_id ] = $class->name;
			}
			...
			?>
            <style>#shipping_package_restrictions .restriction_rows th, #shipping_package_restrictions .restriction_rows td {
                    text-align: center;
                }

                #shipping_package_restrictions .class_name {
                    font-weight: bold;
                    text-align: left;
                }</style>
            <tr valign="top" id="shipping_package_restrictions">
                <th scope="row"
                    class="titledesc"><?php _e( 'Shipping Method Restrictions', 'bolder-multi-package-woo' ); ?>
                ...
                <td class="forminp" id="<?php echo $this->id; ?>_restrictions">
                    <table class="restriction_rows widefat" style="width: 60%;min-width:550px;" cellspacing="0">
                        ...
                        <tfoot>
                        <tr>
                            <td colspan="<?php echo $total_shipping_methods; ?>">
                                <em><?php _e( 'If left blank, all active shipping methods will be used for each shipping class', 'bolder-multi-package-woo' ); ?></em>
                            </td>
                        </tr>
                        </tfoot>
                                ...
                                <tr>
									...
									<?php foreach ( $shipping_methods as $key => $method ) : ?>
                                        ...
                                        <td><input type="checkbox"
                                                   name="restrictions[<?php echo intval( $id ); ?>][<?php echo sanitize_title( $key ); ?>]" <?php echo $checked; ?> />
                                        </td>
									<?php endforeach; ?>
                                </tr>
							<?php
            ...
            <div class="woocommerce_wrap">
                <div style="position:absolute;top:25px;right:10px;width:275px;display:block;padding:10px;border:1px solid #9dbc5a;border-radius:5px;">
                    ...
                    <div style="margin-bottom:25px;">
                        ...
                        <p>Has the ability to return multiple rates based on a variety of conditions such as location,
                            subtotal, shipping class, weight, and more</p>
                        <p style="text-align:right"><a
                                    href="http://codecanyon.net/item/table-rate-shipping-for-woocommerce/3796656?ref=bolderelements"
                                    style="color:#9dbc5a;" target="_blank">More Info</a></p>
                    </div>
                    <div style="margin-bottom:25px;">
                        <strong>Bolder Fees for WooCommerce</strong>
                        <p>Add extra flat rate, percentage, and even optional (checkbox) fees to the customer's cart.
                            Can be based on subtotal, item details, and more</p>
                        <p style="text-align:right"><a
                                    href="http://codecanyon.net/item/bolder-fees-for-woocommerce/6125068?ref=bolderelements"
                                    style="color:#9dbc5a;" target="_blank">More Info</a></p>
                    </div>
                    <div style="">
                        <strong>Cart Based Shipping for WooCommerce</strong>
                        <p>Allows you to change the shipping rate based on the customer's cart. Could be based on the
                            subtotal, item count, or weight.</p>
                        <p style="text-align:right"><a
                                    href="http://codecanyon.net/item/woocommerce-cart-based-shipping/3156515?ref=bolderelements"
                                    style="color:#9dbc5a;" target="_blank">More Info</a></p>
                    </div>
                </div>
			<?php
